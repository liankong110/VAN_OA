...
namespace ImportExcel
{
    public partial class FrmMain : Form
    {
        ...
        private void DoWork(object sheetName)
        {
            int Com = 0;
            var start = DateTime.Now;
            var mysqlTableName = Guid.NewGuid().ToString(); //DateTime.Now.ToString("yyyyMMdd_HH_mm_ss");
            ...
            using (OleDbConnection conn = new OleDbConnection(strConn))
            {
                ...
                using (OleDbDataReader dataReader = objCommand.ExecuteReader())
                {
                    ...
                    while (dataReader.Read())
                    {
                        ...
                        if (allCount % 30000 == 0)
                        {
                            SaveData(dataTable, mysqlTableName);
        ...
        private void Save()
        {
            ...
            foreach (DataRow rowTable in sheetNames.Rows)
            {
                ...
                doWork.Start(rowTable["TABLE_NAME"].ToString());
            }
            conn.Close();
        }

        public void SaveData(DataTable dataTable, string mysqlTableName)
        {
            using (var sqlBulkCopy = new SqlBulkCopy(DBConn))
            {
                sqlBulkCopy.DestinationTableName = "[" + mysqlTableName + "]";
                sqlBulkCopy.BatchSize = dataTable.Rows.Count;
        ...
        private bool CreateTable(string mysqlTableName)
        {
            //lblMess.Text = "创建主表......";
            //Application.DoEvents();
...
) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]", mysqlTableName);
