...
{
    public partial class WFFpTaxInfo : System.Web.UI.Page
    {
        private FpTaxInfoService goodSer = new FpTaxInfoService();
        ...
        {
            base.Response.Redirect("~/BaseInfo/WFFpTaxInfoList.aspx");
        }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            if (this.FormCheck())
            {
                try
                {
                    string sqlCheck = string.Format("select count(*) from FpTaxInfo where FpType='{0}' and id<>{1}",
                  txtFpType.Text, Request["Id"]);
                    ...
                    sqlCheck = string.Format("select count(*) from FpTaxInfo where TAX={0} and id<>{1}",
                txtTax.Text, Request["Id"]);
                    if (Convert.ToInt32(DBHelp.ExeScalar(sqlCheck)) > 0)
                    {
